...
void print_field(bool field[][13]){
	for(int i=0;i<4;i++){
...
void select_player_card(Card player_cards[][13],bool field[][13],int hands_cnt[],int pass[]){
	int num;
	bool check;
	Card tmp;
	queue<Card> work;
...
void select_computer_card(Card player_cards[][13],bool field[][13],int hands_cnt[],int player_num,int pass[]){
...
//		cout << endl;
		for(int i=0;i<4;i++) if(hands_cnt[i] < 1) nohands_cnt++;
		if(nohands_cnt >= 3) break;

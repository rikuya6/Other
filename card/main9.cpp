...
void first_card(Card cards[4][13],const Card deck[52]){ //ゲームの最初にトランプを配る関数。
	int index = 0;
	for(int i=0;i<4;i++){
		for(int k=0;k<13;k++){
...
void select_player1_card(Card cards[4][13],bool field[4][13],int hands_cnt[4],int pass[4]){ //人間プレイヤーが場に出すトランプを選ぶ。
	...
	Card del;
	while(1){
		cout << "左から何番目のカードを出すか入力(-1でパス)" << "パスは" << pass[0] << "回しています。" << endl;
		cout << "入力待ち中：";
		cin >> target;
		if(!cin.good()){
...
int main(){ //ゲームの繰り返し処理と順位表示をする。
	int hands_cnt[4],start_player=1,pass[4]={0},continue_game = 1,rank = 1;
	Rank player_rank[4];
	...
	while(1){
		if(continue_game == 1){ //ゲームの継続判定。
			...
			while(1){ //ゲームの継続処理。
				...
				cout << endl;
				if(continue_game == 1 || continue_game == 0) break; //例外処理
				else{

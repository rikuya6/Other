...
void select_computer_card(Card player_cards[][13],bool field[][13],int hands_cnt[],int player_num){
	int num = 1;
	Card tmp;
	queue<Card> work;
	//cout << player_cards[player_num][num - 1].suit  << ' ' << player_cards[player_num][num - 1].rank << endl;
	field[player_cards[player_num][num - 1].suit][player_cards[player_num][num - 1].rank - 1] = true;
